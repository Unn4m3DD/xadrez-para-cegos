  ...
  generate_table: (target_id, height, width) => {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        old_table[y][x].appendChild(inner_img)
    ...
    for (let x = 0; x < board.length; x++) {
      for (let y = 0; y < board[x].length; y++) {
        if (board[x][y].target) {
          ...
          table[x][y].firstElementChild.style.visibility = "visible"
          table[x][y].firstElementChild.src = `../res/${board[x][y].piece.image}.png`
          ...
          table[x][y].onclick = () => { }
          table[x][y].firstElementChild.style.visibility = "hidden"
          table[x][y].firstElementChild.src = ``
          table[x][y].className = ""
        }
      }
    }
